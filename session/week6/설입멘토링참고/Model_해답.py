...
class TwoLayerNet():
    ...
    def forward(self, X, y=None):
        ...
        a = np.maximum(h,0) # Activation (ReLU)
        scores = np.dot(a,W2) + b2 # Linear 2 
        p = np.exp(scores)/np.sum(np.exp(scores)),axis =1).reshape(-1,1) #Softmax
        if y is None:
            return p, a

        # 여기서 Loss를 구하는 작업을 수행하세요.
		y = y.astype(int)
		log_likellihood = -np.log(p[np.arange(N),y]).sum()
